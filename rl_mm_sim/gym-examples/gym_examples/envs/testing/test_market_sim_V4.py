...
class MarketSimEnv(gym.Env):
    ...
    def __init__(self,
        ...
        # Observation space: top 5 bids price and amount, top 5 asks price and amount, inventory, capital
        self.observation_space = spaces.Box(low=-np.finfo(np.float32).max, high=np.finfo(np.float32).max, shape=(22,), dtype=np.float32)
    ...
    def step(self, action):
        ...
        done = self.minute >= self.max_episode_duration
        info = {}
    ...
    def _apply_action(self, action_type, level, side, price_adjustment, amount_adjustment):
        ...
        if side == 0:  # Bid side
            if action_type == 0:  # Place a new bid
                ...
                new_amount = max(0, 6_000 * (2 + level))  # Adjust based on the amount adjustment
    ...
    def _simulate_market(self):
        num_market_orders = 1#np.random.poisson(5)
        ...
        for _ in range(num_market_orders):
            if np.random.rand() < 0.5:
                if self.asks:
                    ...
                    if best_ask:
                        trade_amount = min(best_ask.amount, np.random.randint(1_000, 5_000))  # random or available amount
                ...
                if self.bids:
                    ...
                    if best_bid:
                        trade_amount = min(best_bid.amount, np.random.randint(1_000, 5_000))
                        self.inventory += trade_amount
    ...
    def _get_observation(self):
        ...
        bids_amounts = [order.amount for order in self.bids]
        asks_prices = [order.price for order in self.asks]
        asks_amounts = [order.amount for order in self.asks]
        observation = np.concatenate([bids_prices, bids_amounts, asks_prices, asks_amounts, [self.inventory, self.capital]])
        return observation.astype(np.float32)

    def render(self, mode='human'):
        print(f"Minute: {self.minute}, Bids: {[{order.price, order.amount} for order in self.bids]}, Asks: {[{order.price, order.amount} for order in self.asks]}, Inventory: {self.inventory}, Capital: {self.capital}, Reward: {self.most_recent_reward}")
    ...
    def basic_reward_function(self):
        ...
        spread_penalty = 0
        found_bid_with_amount = 0
        found_ask_with_amount = 0
        for bid, ask in zip(self.bids, self.asks):
            if bid.amount != 0 and found_bid_with_amount == 0:
                found_bid_with_amount = bid.price
            if ask.amount != 0 and found_ask_with_amount == 0:
                found_ask_with_amount = ask.price
            if found_ask_with_amount != 0 and found_bid_with_amount != 0:
                break

        spread = found_ask_with_amount - found_bid_with_amount
        if spread == 0:
            spread_penalty = 1
        elif spread / found_bid_with_amount > 0.02:
            spread_penalty += 1 * spread

        zero_amount_penalty = sum(5 for order in self.bids + self.asks if order.amount == 0)
        self.most_recent_reward = 1 - spread_penalty
        return 1 - spread_penalty#inventory_penalty - spread_penalty - zero_amount_penalty
