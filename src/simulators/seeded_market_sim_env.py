...
class SeededMarketSimEnv(gym.Env):
    metadata = {"render_modes": ["human"], "render_fps": 30}

    def __init__(self, s0, T, v, q0, c0, sigma, drift, gamma, itp, ms, tf, itfp=1, seed=None, seed_sequence=None):
        '''
        Description:
        ...
        if seed_sequence is not None:
            self.set_seed_sequence(seed_sequence)
        self.reset()

    def reset(self, seed=None, option=None):
        if self.seed_sequence is not None and len(self.seed_sequence) > 0:
    ...
    def set_seed_sequence(self, seed_sequence):
    ...
    def _initiate_price_path(self):
        ...
        for _ in range(1, self.episode_duration):
            random_shock = self.np_random.normal(0, self.sigma * np.sqrt(dt))
    ...
    def _execute_noise_trade(self, bid_price, ask_price, amount):
        if self.np_random.random() < 0.5: # Buy Noise Trade
            self.inventory -= amount
            self.cash += amount * ask_price
            # Stats
            self.total_buys += 1
...
if __name__ == "__main__":
    ...
    #model = DQN('MlpPolicy', env, verbose=1, exploration_fraction=0.1, exploration_final_eps=0.05)
    model = DQN('MlpPolicy', env, verbose=1, exploration_fraction=0.6, exploration_final_eps=0)
